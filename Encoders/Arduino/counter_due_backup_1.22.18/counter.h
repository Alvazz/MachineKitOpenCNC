...
void initCounter(const uint8_t ss[], const int count) {
  ...
  */
  ...
  //for (int iss=0; iss<sizeof(ss)/sizeof(ss[0]); iss++){
  for (int iss=0; iss<(sizeof(ss)/sizeof(ss[0]))+1; iss++){
    Serial.print("iss is ");
    Serial.println(iss,DEC);
    Serial.print("and sizeof(ss) is ");
    Serial.print(sizeof(ss),DEC);
    // set MRD0 and MRD1
    ...
    writeFourBytes(ss[iss], WRITE_DTR, binCount);
    
    //Move written data from DTR to CNTR register
    clrLoadReg(ss[iss], LOAD_CNTR);
  
    Serial.print("writing to SS ");
    Serial.print(ss[iss],DEC);
    Serial.print(" value ");
