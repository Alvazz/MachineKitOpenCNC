...
#PNC Modules
from pncMachineModel import MachineModel, MachineModelProxy#, MachineModelStaticsProxy
...
from pncWebInterface import CloudTrajectoryPlannerInterfaceProcess
...
def appStart(type, pnc_app_manager, machine, synchronizer):
    ...
    cloud_trajectory_planner_interface, tp_pipe_sender = startProcess(machine, synchronizer, CloudTrajectoryPlannerInterfaceProcess)
    ...
    pnc_app_manager.cloud_trajectory_planner_interface = cloud_trajectory_planner_interface
    ...
    closePipes(db_pipe_sender, di_pipe_sender, mc_pipe_sender, fb_pipe_sender, tp_pipe_sender)

    return database, device_interface, feedback_handler, machine_controller, cloud_trajectory_planner_interface
...
def appClose(pnc_app_manager, synchronizer):
    ...
    closeProcess(pnc_app_manager, synchronizer, "cloud_trajectory_planner_interface")
...
def checkSuccessfulStart(pnc_app_manager, synchronizer):
    try:
        ...
        synchronizer.tp_successful_start_event.wait(pnc_app_manager.machine.event_wait_timeout)
        return True
    except:
        return False
...
def waitForAppStart(synchronizer):
    ...
    synchronizer.tp_startup_event.wait()
